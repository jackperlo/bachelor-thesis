...
size_t AleaGame::HashFun::operator()(const AleaGame& game) const {
  ...
  for(int x=0; x<MAP_WIDTH; x++){
    for(int y=0; y<MAP_HEIGHT; y++){
      if(game.dices.find(P2D(x, y)) != game.dices.end()){
        switch(game.dices.at(P2D(x, y))->get_actual_type_int()){
          case 0:
            hash_string.append("W").append((to_string(game.dices.at(P2D(x, y))->get_n_moves())));
            break;
          case 1:
            hash_string.append("R").append((to_string(game.dices.at(P2D(x, y))->get_n_moves())));
            break;
          case 2:
            hash_string.append("Y").append((to_string(game.dices.at(P2D(x, y))->get_n_moves())));
            break;
          case 3:
            hash_string.append("G").append((to_string(game.dices.at(P2D(x, y))->get_n_moves())));
            break;
        }
      }else
        hash_string.append("00");
    }
  }
...
AleaGame& AleaGame::operator=(const AleaGame& game){
  this->heuristic_value = game.heuristic_value;
...
P2D AleaGame::get_new_dice_position(P2D old_position, P2D dir, int n_cells){
  P2D new_position = old_position;

  if(dir == P2D::LEFT) new_position += P2D(-n_cells, 0);
  if(dir == P2D::RIGHT) new_position += P2D(n_cells, 0);
  if(dir == P2D::UP) new_position += P2D(0, -n_cells);
  if(dir == P2D::DOWN) new_position += P2D(0, n_cells);

  return new_position;
}

double AleaGame::calculate_heuristic_value(){
  double heuristic_value = 0.00;
  for(pair<P2D, Dice*> dice : dices){
    for(P2D terminal : terminals){
      if(dice.second->get_n_moves() - dice.first.manatthan(terminal) < 0)
        heuristic_value += (double)((dice.second->get_n_moves() - dice.first.manatthan(terminal)) * -1) / (double)100;
      else 
        heuristic_value += (double)((dice.second->get_n_moves() - dice.first.manatthan(terminal)) % 2) / (double)100;
    }
  }
...
*/
Action AleaGame::move_forward_stats(vector<pair<int, int>> &excluding_heuristic_possible_moves_activation, P2D from_pos, pair<P2D, Dice *> dice, P2D dir, pair<bool, int> move_results, int movement_type) {
  if(dice.second->get_actual_type().compare("WhiteDice") == 0)
    return white_dice_move_forward_stats(from_pos, dice, dir, excluding_heuristic_possible_moves_activation[0], movement_type);
  else if(dice.second->get_actual_type().compare("RedDice") == 0)
    return red_dice_move_forward_stats(from_pos, dice, dir, excluding_heuristic_possible_moves_activation[1], movement_type);
  else if(dice.second->get_actual_type().compare("YellowDice") == 0)
    return yellow_dice_move_forward_stats(from_pos, dice, dir, move_results, excluding_heuristic_possible_moves_activation[2]);
  else if(dice.second->get_actual_type().compare("GreenDice") == 0)
    return green_dice_move_forward_stats(from_pos, dice, dir, move_results, excluding_heuristic_possible_moves_activation[3]);
...
Action AleaGame::white_dice_move_forward_stats(P2D from_pos, pair<P2D, Dice *> dice, P2D dir, pair<int, int> &white_heuristic_stats, int movement_type){
  ...
  if(movement_type == SIMPLE_MOVE)
    ...
    return Action(from_pos, dir, PUSHED_MOVE_FORWARD_WEIGHT, PUSHED_MOVE);       
}
...
Action AleaGame::yellow_dice_move_forward_stats(P2D from_pos, pair<P2D, Dice *> dice, P2D dir, pair<bool, int> move_results, pair<int, int> &yellow_heuristic_stats){
  ...
  if(move_results.second == 1)
    ...
    return Action(from_pos, dir, JUMPING_MOVE_FORWARD_WEIGHT, JUMP_MOVE);  
  }
}
...
bool AleaGame::check_dice_could_be_pushed(P2D dice_position, int dice_moves){
  for(pair<P2D, Dice *> pair : dices)
    if(pair.first != dice_position)
      if(pair.second->get_actual_type().compare("RedDice") == 0 || pair.second->get_actual_type().compare("WhiteDice") == 0)
        if((pair.second->get_n_moves()+dice_moves)>(calculate_distance_from_closer_terminal(dice_position)+dice_position.manatthan(pair.first)))
          return true;
...
bool AleaGame::move(const Action& action, bool is_moving_backward) {
  return move(action.from, action.dir, action.movement_type, is_moving_backward).first;
}
...
vector<pair<AleaGame, vector<Action>>> AleaGame::find_banal_starts_forward_search_wrapper(){
  ...
  for(pair<P2D, Dice*> pair : this->dices){
    for(P2D terminal : this->terminals){
      if((pair.first.manatthan(terminal) == pair.second->get_n_moves() && pair.first!=terminal) || (pair.second->get_n_moves() > pair.first.manatthan(terminal) && pair.second->get_n_moves()%2 == pair.first.manatthan(terminal)%2)){
            ...
            for(std::pair<P2D, Dice*> d : disputer_dices_result){
              ...
              if(find_banal_start_calculate_route(res.second, d, d.second->get_n_moves(), terminal, res.first.dices)){
                for(Action action : res.second){
                  ...
                  res.first.heuristic_value += res.first.calculate_heuristic_value();
                }
              ...
              if(find_banal_start_calculate_route(actions, d, d.second->get_n_moves(), terminal, moves.first.dices)){                
                for(Action action : actions){
                  if(moves.first.dices.find(action.from) != moves.first.dices.end()){
                    ...
                    moves.first.heuristic_value += moves.first.calculate_heuristic_value();
                  }
                }
...
bool AleaGame::find_banal_start_calculate_route(vector<Action> &moves, pair<P2D, Dice*> dice, int dice_moves, P2D terminal_position, unordered_map<P2D, Dice *, P2D::HashFun> dices){
  ...
  if(dice.first.x < terminal_position.x){//moving RIGHT
    if(dice.second->get_actual_type().compare("GreenDice") == 0){
      ...
      if(dice_moves-(i-1) > 0 && (dices.find(dice.first+P2D(i, 0)) == dices.end() && dice.first.x+i < MAP_WIDTH)){
        while(dice_moves-(i-1) > 0 && (dices.find(dice.first+P2D(i, 0)) == dices.end() && dice.first.x+i < MAP_WIDTH))
          i++;
        moves.push_back(Action(dice.first, P2D::RIGHT, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE));
      ...
      if(dices.find(dice.first+P2D::RIGHT) == dices.end()){
        ...
        if(dices.find(dice.first+P2D::UP) == dices.end() && (dice.first+P2D::UP).y >= 0){ 
          moves.push_back(Action(dice.first, P2D::UP, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE));
          dice.first=dice.first+P2D::UP;
          return find_banal_start_calculate_route(moves, dice, dice_moves-1, terminal_position, dices);
        }else if(dices.find(dice.first+P2D::DOWN) == dices.end() && (dice.first+P2D::DOWN).y < MAP_HEIGHT){
          moves.push_back(Action(dice.first, P2D::DOWN, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE));
    ...
    if(dice.second->get_actual_type().compare("GreenDice") == 0){
      ...
      if(dice_moves-(i-1) > 0 && (dices.find(dice.first+P2D(-i, 0)) == dices.end() && dice.first.x-i >= 0)){ //if it can move at least by one
        while(dice_moves-(i-1) > 0 && (dices.find(dice.first+P2D(-i, 0)) == dices.end() && dice.first.x-i >= 0))
          i++;
        moves.push_back(Action(dice.first, P2D::LEFT, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE)); 
        dice.first=dice.first+P2D(-(i-1), 0);
        return find_banal_start_calculate_route(moves, dice, dice_moves-(i-1), terminal_position, dices);
      }else
        return false;
  ...
  }else if(dice.first == terminal_position){
    if(dices.find(dice.first+P2D::DOWN) == dices.end() && (dice.first+P2D::DOWN).y < MAP_HEIGHT){
      moves.push_back(Action(dice.first, P2D::DOWN, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE)); 
      dice.first=dice.first+P2D::DOWN;
      return find_banal_start_calculate_route(moves, dice, dice_moves-1, terminal_position, dices);
    }else 
    ...
    if(dices.find(dice.first+P2D::RIGHT) == dices.end() && (dice.first+P2D::RIGHT).x < MAP_WIDTH){
      moves.push_back(Action(dice.first, P2D::RIGHT, SIMPLE_MOVE_FORWARD_WEIGHT, SIMPLE_MOVE));
      dice.first=dice.first+P2D::RIGHT;
      return find_banal_start_calculate_route(moves, dice, dice_moves-1, terminal_position, dices);
    }else
...
Action AleaGame::revert_action(Action backward_action){
  return Action(backward_action.head, P2D(backward_action.dir.x*-1, backward_action.dir.y*-1), backward_action.heuristic_value, backward_action.movement_type);
}
